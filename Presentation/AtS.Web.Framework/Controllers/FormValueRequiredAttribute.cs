...
namespace TWYK.Web.Framework.Controllers
{
    /// <summary>
                    ...
                    switch (_requirement) {
                        case FormValueRequirement.Equal: {
                            if (_validateNameOnly) {
                                //"name" only
                                if (controllerContext.HttpContext.Request.Form.AllKeys.Any(x =>
                                    x.Equals(buttonName, StringComparison.InvariantCultureIgnoreCase))) {
                                    return true;
                                }
                            }
                            else {
                        ...
                        case FormValueRequirement.StartsWith: {
                            if (_validateNameOnly) {
                                //"name" only
                                if (controllerContext.HttpContext.Request.Form.AllKeys.Any(x =>
                                    x.StartsWith(buttonName, StringComparison.InvariantCultureIgnoreCase))) {
                                    return true;
                                }
                            }
                            else {
