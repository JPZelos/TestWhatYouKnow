...
{
    /// <summary>
    /// A class that finds types needed by AthleticShop by looping assemblies in the
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        /// <summary>
        /// Gets or sets whether AthleticShop should iterate assemblies in the app domain when loading AthleticShop types. Loading
        /// patterns
        ...
        /// </summary>
        /// <remarks>
        /// If you change this so that AthleticShop assemblies arn't investigated (e.g. by not including something like
        /// "^AthleticShop|..."
                    ...
                    if (types != null)
                        foreach (var t in types)
                            if (assignTypeFrom.IsAssignableFrom(t) || assignTypeFrom.IsGenericTypeDefinition &&
                                DoesTypeImplementOpenGeneric(t,
                                    assignTypeFrom))
                                if (!t.IsInterface)
        ...
        /// <returns>A list of assemblies that should be loaded by the AthleticShop factory.</returns>
        ...
        /// </param>
        /// <returns>
        /// True if the assembly should be loaded into AthleticShop.
        /// </returns>
        public virtual bool Matches(string assemblyFullName) {
            return !Matches(assemblyFullName,
                       AssemblySkipLoadingPattern)
                   && Matches(assemblyFullName,
        ...
        /// <returns></returns>
        protected virtual bool DoesTypeImplementOpenGeneric(
            Type type,
            Type openGeneric
        ) {
            try {
                ...
                foreach (var implementedInterface in type.FindInterfaces((
                        objType,
                        objCriteria
                    ) => true,
                    null)) {
